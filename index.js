...
const commands = {
        ...
        (function play(song) {
            ...
            collector.on('message', m => {
                if (m.content.startsWith(tokens.prefix + 'pause')) {
                    msg.channel.sendMessage('РџРѕСЃС‚Р°РІР»РµРЅРЅРѕ РЅР° РїР°СѓР·Сѓ').then(() => {
                        dispatcher.pause();
                    });
                } else if (m.content.startsWith(tokens.prefix + 'resume')) {
                    msg.channel.sendMessage('РЎРЅСЏС‚Рѕ СЃ РїР°СѓР·С‹').then(() => {
                        dispatcher.resume();
                    });
                } else if (m.content.startsWith(tokens.prefix + 'skip')) {
                    msg.channel.sendMessage('РџСЂРѕРїСѓС‰РµРЅРЅРѕ').then(() => {
                        dispatcher.end();
                    });
    ...
    'queue': (msg) => {
        ...
        queue[msg.guild.id].songs.forEach((song, i) => {
            tosend.push(`${i + 1}. ${song.title} - Р—Р°РїСѓС‰РµРЅРѕ: ${song.requester}`);
        });
...
client.on('guildMemberAdd', member => {
    try {
        ...
        console.log(`РћС€РёР±РєР° API: ${error}`);
    }
});
client.on("ready", () => {
    ru_initialize();
...
if (command === "testnewuser") {
    ...
    try {
        ...
        message.reply(`РћС€РёР±РєР° API: ${error}`);
    }
}
